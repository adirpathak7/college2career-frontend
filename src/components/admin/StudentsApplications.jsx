...
export default function StudentsApplications() {
        ...
        const fetchApplications = async () => {
            try {
                const response = await axios.get(
                    `${import.meta.env.VITE_BASE_URL}/users/students/getStudentByPendingStatus`
                )
    ...
    const handleApprove = async (studentId) => {
        try {
            setLoading(true)
            const response = await axios.patch(`${import.meta.env.VITE_BASE_URL}/users/students/updateStudentStatus/1`, {
                studentId,
                status: "activated",
            })
            ...
            if (response.data.status) {
                ...
                setApplications(prev => prev.filter(c => c.studentId !== studentId))
            } else {
        ...
        <div className="w-full min-h-screen bg-gray-900 text-white py-10 px-4 md:px-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {applications.map(data => (
                    <div key={data.studentId} className="bg-gray-800 rounded-xl p-6 shadow-lg space-y-3">
                        <div>
                            ...
                            <p className="text-purple-300 text-sm">
                                {data.course} • {data.graduationYear}
                            </p>
                            {/* <p className="text-sm text-gray-400">Resume: {data.resume}</p> */}
                            <iframe src={data.resume} frameborder="0">
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => handleApprove(data.studentId)}
                            ...
                            <button
                                onClick={() => handleReject(data.studentId)}
